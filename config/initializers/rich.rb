...
if Object.const_defined?('Rich')
  Rich.setup do |config|
    ...
    config.editor[:toolbar] = [%w(Styles Format Font FontSize), %w(Bold Italic Underline Strike Subscript Superscript),
                               %w(JustifyLeft JustifyCenter JustifyRight JustifyBlock), %w(TextColor BGColor),
                               %w(RemoveFormat), %w(NumberedList BulletedList Blockquote), %w(Link Unlink), %w(SpecialChar),
                               %w(richImage MediaEmbed), %w(Source ShowBlocks)]
