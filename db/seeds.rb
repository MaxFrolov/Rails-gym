...
12.times do
  ...
  User.find_by(first_name: 'Max').posts.create ({picture_url: File.open(File.join(Rails.root, 'test', 'fixtures' , 'blog-img', "blog_picture#{post_incr += 1}.jpg")),
  ...
  DietAdvice.create({ category: Faker::Lorem.word, header: Faker::Lorem.sentence(1), short_description: Faker::Lorem.sentence(5),
                   image: File.open(File.join(Rails.root, 'test', 'fixtures' , 'nutrition', "#{nutrition_incr += 1}.jpg")), description: Faker::Lorem.paragraph(8) })
end
